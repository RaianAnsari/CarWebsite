            ...
            <div class="flex">
                ...
                <div class="flex items-center shrink-0">
                    <a href="{{ route('dashboard') }}" class="flex items-center space-x-2">
                        <x-application-logo class="block w-auto text-blue-600 fill-current h-9 dark:text-blue-500" />
                        <span class="text-xl font-bold text-gray-900 dark:text-white">Automobile Sell, Buy & Maintenance Website</span>
                    </a>
                </div>
                            ...
                            <x-slot name="trigger">
                                <button class="inline-flex items-center px-3 py-2 text-sm font-medium leading-4 text-gray-500 transition duration-150 ease-in-out bg-white border border-transparent rounded-md dark:text-gray-400 dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none">
                                    <div class="flex items-center">
                                        <img class="object-cover w-8 h-8 mr-2 rounded-full" src="{{ Auth::user()->profile_photo_url ?? 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name) }}" alt="{{ Auth::user()->name }}">
            ...
            @auth
                ...
                @else
                    ...
                    <x-responsive-nav-link :href="route('notifications.index')" :active="request()->routeIs('notifications.index')">
                        {{ __('Notifications') }}
                        @if($unreadCount > 0)
                            <span class="px-2 py-1 ml-2 text-xs text-white bg-red-500 rounded-full">
                                {{ $unreadCount }}
                            </span>
                        @endif
                    </x-responsive-nav-link>
